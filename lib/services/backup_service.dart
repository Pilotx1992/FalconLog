...
class BackupService {
  ...
  static Future<BackupResult> backupToGoogleDrive(List<FlightLog> logs) async {
    try {
      ...
      // Create file metadata - Store in main Drive (visible to user like WhatsApp)
      final driveFile = drive.File();
      driveFile.name = fileName;
      driveFile.description = 'FalconLog Flight Data Backup';
      // Remove appDataFolder to make files visible in user's Drive
      // driveFile.parents = ['appDataFolder']; // This makes files hidden
      
      // Create or find FalconLog folder in user's Drive
      ...
      // Set the folder as parent (makes backup visible in user's Drive)
      if (folderId != null) {
      ...
      if (response.id != null) {
        ...
        return BackupResult.success(
          message: 'Successfully backed up to Google Drive\n📁 Check "FalconLog Backups" folder in your Drive',
          logsCount: logs.length,
          backupSize: dataBytes.length,
          filePath: 'Google Drive: FalconLog Backups/$fileName',
        );
      } else {
        return BackupResult.error('Failed to upload to Google Drive');
      }
    } catch (e) {
