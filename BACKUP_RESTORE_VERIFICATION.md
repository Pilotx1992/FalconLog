...
**النظام يدعم استرجاع الحساب بعد إعادة التثبيت عند توفر النسخة والمفاتيح على Google Drive** — يتطلب اختباراً على أجهزة حقيقية وتوقيع Release قبل الاعتماد للإنتاج.
...
**الأمان:**
- 🔒 تشفير AES-256-GCM (authenticated encryption)
...
|--------|---------|-----------|
| **الأمان** | ⭐⭐⭐⭐ | AES-256-GCM للحمولة؛ مفتاح الاسترداد في Google AppData (ليس E2E ضد اختراق حساب Google) |
...
### ⚠️ حالة الجاهزية (محدّث)

نظام Backup/Restore يدعم استرجاع الحساب عبر Google Drive AppData مع تشفير AES-256-GCM للحمولة. **Replace و Merge** يستخدمان journal + snapshot للتراجع بعد PR2. **الاحتفاظ بأحدث نسخة** يبقي أحدث نسخة ناجحة بعد اكتمال نسخ جديد. الجاهزية للإنتاج تعتمد على توقيع Release، واختبار أجهزة حقيقية، وPlay/internal testing — راجع `docs/EXISTING_USER_UPDATE.md`.

### المميزات الرئيسية:
✅ استرجاع مفاتيح من Google Drive AppData (نفس حساب Google)
✅ تشفير AES-256-GCM للحمولة
✅ Replace/Merge: journal + snapshot (PR2)
✅ أحدث نسخة احتياطية بعد النجاح (PR2.2)
✅ App lock يحمي واجهة التطبيق فقط؛ Hive محلي غير مشفّر في هذا الإصدار

### التوصيات:
1. اختبار ترقية من إصدار مثبّت + نسخ احتياطي يدوي قبل التحديث
